...
from losses.main import transpos
...
def textToBert_dim96(text):
    tokenizer = BertTokenizer.from_pretrained(r'./runners/bertbasecased/vocab.txt')
    tokens = tokenizer.tokenize(text)
    tokens = ["[CLS]"] + tokens + ["[SEP]"]
    ids = torch.tensor([tokenizer.convert_tokens_to_ids(tokens)])
    model = BertModel.from_pretrained(r'./runners/bertbasecased/')
...
def conditioning_fn(config, X, num_frames_pred=0, prob_mask_cond=0.0, prob_mask_future=0.0, conditional=True):
    ...
    # imsize_2 = config.data.image_size_2
    if not conditional:
        # 2 5 3 128 128  -----  b f c h w
        return X.reshape(len(X), -1, imsize, imsize), None, None  # b (f c) h w
    cond = config.data.num_frames_cond #
    train = config.data.num_frames
    pred = num_frames_pred # 10
...
def get_gaus_model(config):
    from models.gaussianDf import GaussianDiffusion
    return GaussianDiffusion(image_size=128, num_frames=7, timesteps=1000, ).to(config.device)
...
class NCSNRunner():
    ...
    def train(self):
        ...
        # self.config.input_dim = self.config.data.image_size  * self.config.data.image_size_2 *  self.config.data.channels
        self.config.input_dim = self.config.data.image_size ** 2 * self.config.data.channels

        # [model] auotoencoder
        ddconfig = {'double_z': True, 'z_channels': 4, \
                    'resolution': 128, 'in_channels': 3, \
                    'out_ch': 3, 'ch': 128, 'ch_mult': [1, 2, 4, 4], \
                    'num_res_blocks': 2, 'attn_resolutions': [], 'dropout': 0.0}
        autoencoder = AutoencoderKL(ddconfig, 4, ckpt_path=self.config.model.sd_checkpoint)
        autoencoder.eval()
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            for batch, (X, y,d,m,p) in enumerate(dataloader):
                texts = []
                for text in y:
                    texts.append(text)

                texts = torch.cat(texts).to(self.config.device)
                optimizer.zero_grad()
                ...
                loss = anneal_dsm_score_estimation(scorenet, X,labels=None, cond=cond, cond_mask=cond_mask,
                                                   ...
                                                   texts = texts,deepth_video = d,motion = m,pose_video = p)
                ...
                if step == 1 or step % self.config.training.val_freq == 0:
                    try:
                        test_X, test_y,d,m,p = next(test_iter)
                    ...
                    with torch.no_grad():
                        test_dsm_loss = anneal_dsm_score_estimation(test_scorenet,test_X, labels=None,
                                                                    cond=test_cond, cond_mask=cond_mask,
                                                                    ...
                                                                    texts=texts,deepth_video = mask,motion = mask,pose_video = mask)
                ...
                if (
                    ...
                    if  step % self.config.training.snapshot_freq == 0 and self.config.training.snapshot_sampling:  # only at snapshot_freq, not at sample_freq
                    ...
                    if conditional:
                        ...
                        test_X, test_cond, test_cond_mask = conditioning_fn(self.config, test_X,
                                                                            ...
                                                                            prob_mask_future=getattr(self.config.data,
                                                                                                     ...
                                                                                                     0.0),
                                                                            conditional=conditional)
                    all_samples = sampler(init_samples, test_scorenet, cond=test_cond, cond_mask=cond_mask,
                                          ...
                                          gamma=getattr(self.config.model, 'gamma', False),texts = texts,deepth_video = d,ori_X =X,motion = m,pose_video = p).to('cpu')
                    ...
                    if conditional:
                        ...
                        data = torch.cat(
                            [ padding_green, padding_red, pred, padding_red], dim=-1)
                        nrow = ceil(np.sqrt((
                                                    self.config.data.num_frames_cond + self.config.data.num_frames * 2 + future) * n_init_samples) / (
                                            self.config.data.num_frames_cond + self.config.data.num_frames * 2 + future))
        ...
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                all_samples = sampler(init_samples, scorenet, cond=cond, cond_mask=cond_mask,
                                      ...
                                      subsample_steps=getattr(self.config.sampling, 'subsample', None),
                                      clip_before=getattr(self.config.sampling, 'clip_before', True),
                                      log=True, gamma=getattr(self.config.model, 'gamma', False),texts = text,deepth_video = d,motion = m,pose_video = p).to('cpu')
            ...
            for i in tqdm(range(n_rounds), desc='Generating samples for FID'):
                ...
                if conditional and not self.config.sampling.data_init:
                    try:
                        real, texts,d = next(data_iter_cond)
                    except StopIteration:
                        if self.config.data.dataset.upper() == 'FFHQ':
                            dataloader = FFHQ_TFRecordsDataLoader([self.args.data_path],
                                                                  self.config.sampling.batch_size,
                ...
                all_samples = sampler(init_samples, scorenet, cond=cond,
                                      ...
                                      subsample_steps=getattr(self.config.sampling, 'subsample', None),
                                      clip_before=getattr(self.config.sampling, 'clip_before', True),
                                      log=True, gamma=getattr(self.config.model, 'gamma', False),texts = texts,deepth_video = d).to('cpu')
    ...
    def video_gen(self, scorenet=None, ckpt=None, train=False):
        ...
        if calc_fvd:
            ...
            if self.condp == 0.0 and self.futrf == 0:  # (1) Prediction
                calc_fvd1 = self.condf + self.config.sampling.num_frames_pred >= 5
        ...
        for i, (real_, y,d,m,p) in tqdm(enumerate(dataloader), total=min(max_data_iter, len(dataloader)),
            ...
            for i_frame in tqdm(range(n_iter_frames), desc="Generating video frames"):
                # scorenet = partial(scorenet, cond=texts)
                mynet = scorenet
                ...
                gen_samples = sampler(init_samples if i_frame == 0 or getattr(self.config.sampling, 'init_prev_t',
                                      ...
                                      gamma=getattr(self.config.model, 'gamma', False),texts = texts,deepth_video = d,motion = m,is_video_gen=is_video_gen,pose_video = p)
            ...
            if future > 0 and prob_mask_future > 0.0 and not self.prob_mask_sync:
                ...
                if real.shape[1] < pred.shape[1]:  # We cannot calculate MSE, PSNR, SSIM
                    ...
                    # Calculate MSE, PSNR, SSIM
                    print('Now  Calculate MSE, PSNR, SSIM')
            ...
            #     cond = cond_original
                ...
                for t in range(cond.shape[1] // self.config.data.channels):
                    cond_t = cond[:, t * self.config.data.channels:(t + 1) * self.config.data.channels].cpu()  # BCHW
                    frame = torch.cat([cond_t, 0.5 * torch.ones(*cond_t.shape[:-1], 2), cond_t], dim=-1)
                    frame = frame.permute(0, 2, 3, 1).numpy()
                ...
                def save_pred(pred, real):
                    ...
                    cond_im = stretch_image(cond, self.config.data.channels, self.config.data.image_size).cpu()
                    pred_im = stretch_image(pred, self.config.data.channels, self.config.data.image_size).cpu()
                    real_im = stretch_image(real, self.config.data.channels, self.config.data.image_size).cpu()
    ...
    def test(self):
        ...
        for ckpt in tqdm(range(self.config.test.begin_ckpt, self.config.test.end_ckpt + 1,
            ...
            for x, y,d,m,p in test_dataloader:
                step += 1
                texts = []
                for text in y:
                    # text = textToBert_dim96(str(text)).detach()
                    # text = torch.tensor(text)
                    texts.append(text)
                x = x.to(self.config.device)
                x = data_transform(self.config, x)
